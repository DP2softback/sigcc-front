export const GAPS_ANALYSIS_MODULE = 'gaps-analysis'; // NOMBRE DEL MODULO 2
export const INDEX = 'index';

export const COMPETENCES = 'competences-management';

export const GAPS_EMPLOYEES_ORG = 'employees-gaps';
export const GAPS_EMPLOYEES_ORG_DETAIL = 'detail';
export const GAPS_EMPLOYEES_ORG_DETAIL_EMPLOYEE = 'more';

export const EMPLOYEES_JOB_OPPORTUNITIES = 'employees-job-opportunities';
// export const A

export const GAPS_EMPLOYEE_EMP = 'my-gaps';
export const GAPS_EMPLOYEE_EMP_DETAIL = 'detail';

export const MY_JOB_OPPORTUNITIES = 'my-job-opportunities;'

